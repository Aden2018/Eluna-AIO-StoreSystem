...
local CONFIG = {
    maxCategories = 11,
    -- Font used for all store text. Defaults to the client's own UI font so
    -- Chinese (and other non-Latin glyphs) render correctly regardless of
    -- client locale or font patch. If you still see "?" after reloading, set
    -- this to a CJK font your client actually ships, e.g. "Fonts\\ZYKai_T.ttf",
    -- "Fonts\\ARKai_T.ttf", "Fonts\\ZYHei.ttf" or "Fonts\\2002.ttf".
    font = (GameFontNormal and select(1, GameFontNormal:GetFont())) or "Fonts\\ZYKai_T.ttf",
    -- Opacity of the store window background (0 = fully transparent, 1 = fully
    -- opaque). Set below 1 so the game world shows through the dialog. Buttons
    -- and text stay fully readable because only the background texture fades.
    frameAlpha = 0.85,
    strings = {
...
function SHOP_UI.MainFrame_Create()
    ...
    -- Semi-transparent background so the game world shows through. Only the
    -- background texture fades; child buttons/text remain fully readable.
    shopFrame.Background:SetAlpha(CONFIG.frameAlpha)

    -- Make the window draggable by the left mouse button. Only empty areas of
    -- the background are draggable: child buttons still receive their clicks
    -- because WoW dispatches mouse events to the topmost child first.
    shopFrame:SetMovable(true)
    shopFrame:EnableMouse(true)
    shopFrame:RegisterForDrag("LeftButton")
    shopFrame:SetScript("OnDragStart", function(self) self:StartMoving() end)
    shopFrame:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end)

    --Title--
    shopFrame.Title = shopFrame:CreateFontString()
    shopFrame.Title:SetFont(CONFIG.font, 14)
    shopFrame.Title:SetShadowOffset(1, -1)
    shopFrame.Title:SetPoint("TOP", shopFrame, "TOP", 0, -3)
    shopFrame.Title:SetText("|cffedd100商城|r")
...
function SHOP_UI.NavButtons_Create(parent)
    ...
    for i = 1, 12 do
        ...
        navButton.Name:SetFont(CONFIG.font, 14)
...
function SHOP_UI.ServiceBoxes_Create(parent)
    ...
    for i = 1, 8 do
        ...
        service.NameFont:SetFont(CONFIG.font, 11)
        ...
        service.PriceFont:SetFont(CONFIG.font, 13)
        ...
        --Discount--
        service.DiscountFont = service:CreateFontString()
        service.DiscountFont:SetFont(CONFIG.font, 10)
        service.DiscountFont:SetShadowOffset(1, -1)
        service.DiscountFont:SetPoint("CENTER", service.PriceFont, "CENTER", -30, 0)

        --Discount Line--
        service.DiscountSlash = service:CreateTexture(nil, "OVERLAY")
        service.DiscountSlash:SetSize(18, 18)
        service.DiscountSlash:SetPoint("CENTER", service.DiscountFont)
        ...
        service.BannerText:SetFont(CONFIG.font, 11)
        ...
        service.buyButton.ButtonText:SetFont(CONFIG.font, 11, "OUTLINE")
        ...
        service.buyButton:SetScript(
            ...
            function(self)
                ...
                PlaySound("igMainMenuOpen", "Master")
            end
        )
        ...
        service:SetScript(
            "OnClick",
            function(self)
                if(self.Type == 1 and self.Flags == 1) then
...
function SHOP_UI.ServiceBoxes_Update()
    ...
    for i, serviceData in ipairs(services) do
        if i >= startIndex and i <= endIndex then
            ...
            service.DiscountFont:SetFormattedText("|cffdbe005%i|r", service.Price)
            service.currencyIcon:SetTexture("Interface/Store_UI/Currencies/" .. currencyIcon)

            -- calculate discount percentage
            local discountPct = math.floor( service.Discount / service.Price * 100 )
            service.BannerText:SetFormattedText("|cffffffffOn sale: %i%% OFF|r", discountPct)
...
function SHOP_UI.PageButtons_Create(parent)
    ...
    pageText:SetFont(CONFIG.font, 13)
...
function SHOP_UI.CurrencyBadges_Create(parent)
    ...
    for i = 1, 4 do
        ...
        currencyButton.Amount:SetFont(CONFIG.font, 10, "OUTLINE")
...
function SHOP_UI.ModelFrame_ShowPlayer(rewardList)
    ...
    if(rewardList) then
        for _, id in pairs(rewardList) do
            if(id > 0) then
...
-- Default game menu buttons in their conventional top-to-bottom order.
-- Used by ReflowGameMenu so the menu can be restacked cleanly whenever it opens.
local GAME_MENU_BUTTON_ORDER = {
...
-- Stacks every button currently parented to the game menu (the Store button,
-- the default buttons, AND any buttons other addons may have added) top to
-- bottom with a fixed gap, so absolute-position overlaps can no longer occur.
-- Re-runs on every OnShow, so it stays correct regardless of load order or of
-- when/if other addons create their own menu button.
local function ReflowGameMenu()
    ...
    -- Store button on top, then the known default buttons, then any extra
    -- button children other addons have added (e.g. an AddOns button).
    add(_G["GameMenuButtonStore"])
    for _, name in ipairs(GAME_MENU_BUTTON_ORDER) do
        add(_G[name])
    end
    ...
    local TOP_OFFSET = -27   -- first button's top edge distance from frame top
    local GAP = 2            -- gap between buttons (in the BOTTOM anchor offset)
    local btnH = 21

    ordered[1]:ClearAllPoints()
    ordered[1]:SetPoint("TOP", frame, "TOP", 0, TOP_OFFSET)

    for i = 2, #ordered do
        ordered[i]:ClearAllPoints()
        ordered[i]:SetPoint("TOP", ordered[i - 1], "BOTTOM", 0, -GAP)
        btnH = math.max(btnH, ordered[i]:GetHeight())
    end

    -- Grow the frame so the bottom button always fits inside it.
    local contentH = -TOP_OFFSET + #ordered * btnH + (#ordered - 1) * GAP + 16
    frame:SetSize(195, contentH)
end
...
local function ModifyGameMenuFrame()
    ...
    -- Create the Store button with a global name so ReflowGameMenu (and any
    -- other addon's reflow) can discover and stack it.
    local storeButton = _G["GameMenuButtonStore"] or CreateFrame("Button", "GameMenuButtonStore", frame, "UIPanelButtonTemplate")
    storeButton:SetSize(144, 21)
    if not storeButton.Text then
    storeButton.Text = storeButton:CreateFontString()
    storeButton.Text:SetFont(CONFIG.font, 14, "OUTLINE")
    storeButton.Text:SetShadowOffset(1, -1)
    storeButton.Text:SetPoint("CENTER", storeButton, "CENTER", 0, 1)
    end
    storeButton.Text:SetText("|cffdbe005商城|r")
    ...
    -- Reflow now and every time the menu opens so we never overlap another
    -- addon's button, no matter who created their button when.
    ReflowGameMenu()
    frame:HookScript("OnShow", ReflowGameMenu)
end
